...
def cfg():
    ...
    return {
        ...
        "background_music_volume": parse_float(os.environ.get("BACKGROUND_MUSIC_VOLUME"), 0.035),
...
def ytdlp_common_args():
    ...
    js_runtimes = os.environ.get("YTDLP_JS_RUNTIMES", "deno").strip()
...
def select_music_track(config, clip):
    ...
    if raw_file and raw_file.lower() not in {"auto", "theme", "content"}:
        return {
            ...
            "label": "Manual",
            "file": raw_file,
            "volume": config.get("background_music_volume", 0.035),
        }, None
...
def apply_background_music(video_path, config, job_id, index, clip=None):
    ...
    track_volume = parse_float(music_selection.get("volume"), config.get("background_music_volume", 0.035))
